...
#include "Controlers/WarriorHeroController.h"
...
void UHeroGameplayAbility_TargetLock::OnTargetLockTick(float DeltaTime)
{
	...
	!UWarriorFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), WarriorGameplayTags::Player_Status_Roll);
	// &&
...
void UHeroGameplayAbility_TargetLock::SwitchTarget(const FGameplayTag& InSwitchDirectionTag)
{
	...
	AActor* NewTargetLock = nullptr;
	
	GetAvailableActorsAroundTarget(ActorsOnLeft, ActorsOnRight);

	if (InSwitchDirectionTag == WarriorGameplayTags::Player_Event_SwitchTarget_Left)
	{
		NewTargetLock = GetNearestTargetAvailableActors(ActorsOnLeft);
	}
	else
	{
		NewTargetLock = GetNearestTargetAvailableActors(ActorsOnRight);
	}

	if (NewTargetLock)
	{
		CurrentLockedActor = NewTargetLock;
	}
}
...
void UHeroGameplayAbility_TargetLock::TryLockOnTarget()
{
	...
	CurrentLockedActor = GetNearestTargetAvailableActors(AvailableActorsToLock);
...
AActor* UHeroGameplayAbility_TargetLock::GetNearestTargetAvailableActors(const TArray<AActor*>& InAvailableActors)
{
...
void UHeroGameplayAbility_TargetLock::InitTargetLockMappingContext()
{
	...
	Subsystem->AddMappingContext(TargetLockMappingContextContext, 3);
}
...
void UHeroGameplayAbility_TargetLock::ResetTargetLockMappingContext()
{
	...
	Subsystem->RemoveMappingContext(TargetLockMappingContextContext);
}