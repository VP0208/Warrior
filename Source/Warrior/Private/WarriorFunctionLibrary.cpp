	...
	float& OutAngelDifference)
{
	...
	OutAngelDifference = UKismetMathLibrary::DegAcos(DotResult);

	const FVector CrossResult = FVector::CrossProduct(VictimForward, VictimToAttackerNormalized);
	
	if (CrossResult.Z < 0.f) OutAngelDifference *= -1.f;

	if (OutAngelDifference >= -45.f && OutAngelDifference <= 45.f) return WarriorGameplayTags::Shared_Status_HitReact_Front;

	if (OutAngelDifference < -45.f && OutAngelDifference >= -135.f) return WarriorGameplayTags::Shared_Status_HitReact_Left;

	if (OutAngelDifference < -135.f || OutAngelDifference > 135.f ) return WarriorGameplayTags::Shared_Status_HitReact_Back;

	if (OutAngelDifference > 45 && OutAngelDifference <= 135) return WarriorGameplayTags::Shared_Status_HitReact_Right;
	...
	const FGameplayEffectSpecHandle& InSpecHandle)
{
	...
	FActiveGameplayEffectHandle ActiveGameplayEffect = SourceASC->ApplyGameplayEffectSpecToTarget(*InSpecHandle.Data,TargetASC);

	return ActiveGameplayEffect.WasSuccessfullyApplied();
}
...
bool UWarriorFunctionLibrary::TryLoadSavedDifficulty(EWarriorGameDifficulty& OutSavedGameDifficulty)
{
	if(UGameplayStatics::DoesSaveGameExist(WarriorGameplayTags::GameData_SaveGame_Slot_1.GetTag().ToString(), 0))
	{
		...
		if (UWarriorSaveGame* WarriorSaveGameObject = Cast<UWarriorSaveGame>(SaveGameObject))
		{
			OutSavedGameDifficulty = WarriorSaveGameObject->SavedCurrentGameDifficulty;
